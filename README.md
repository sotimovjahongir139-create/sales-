...
Frontend and backend deploy as two separate services (e.g. frontend on Vercel, backend
wherever it can run a persistent Node process — Vercel's serverless functions aren't a
fit for this backend as-is, since it needs a long-running process for the
`node-cron` sync scheduler and a normal TCP connection to Postgres).

```bash
cd frontend && npm run build   # outputs frontend/dist, deploy as a static site
cd backend && npm start        # run behind a process manager (pm2, systemd, a Node host)
```
